...
type ProjectManagerLike = Pick<
  ProjectManager,
  "loadProjects" | "projectCount" | "projects"
> & {
...
export class InitializationManager {
  ...
  initialize(): ResultAsync<void, MainAppViewError> {
    ...
    return (
      NeverthrowResultAsync.combine([
        ...
        .andThen((restoredSessionIds) =>
          this.hydrateStartupPanels(restoredSessionIds),
        )
      ...
      return errAsync(
        new InitializationError(
          ...
          initResult.error instanceof Error
            ? initResult.error
            : new Error(String(initResult.error)),
        ),
      );
    }
    ...
    const loadAgentsAndWarmPreconnectionCommands = this.agentStore
      ...
      .andThen((agents) =>
        this.preconnectionAgentSkillsStore
          ...
          .orElse((error) => {
            logger.warn(
              "Failed to warm preconnection agent skills; continuing startup",
              {
                error,
              },
            );
   ...
   */
  private remapAliasedPanelSessionIds(
  ...
  private validateRestoredSessions(): ResultAsync<void, MainAppViewError> {
    ...
    for (const panel of this.panelStore.panels) {
      if (
        panel.sessionId &&
        !this.sessionStore.getSessionCold(panel.sessionId)
      ) {
    ...
    if (clearedCount > 0) {
      logger.info(
        `Cleared ${clearedCount} orphaned session reference(s) from panels`,
      );
    }
    return okAsync(undefined);
  }
   ...
   * @returns ResultAsync indicating success or error
   */
  private createSessionsForAgentOnlyPanels(): ResultAsync<
    void,
    MainAppViewError
  > {
    ...
    const panelsNeedingSessions = this.panelStore.panels.filter(
      (p) =>
        p.selectedAgentId && !p.sessionId && p.selectedAgentId !== "opencode",
    );
