...
describe("InitializationManager", () => {
  ...
  beforeEach(() => {
    ...
    mockSessionStore = {
      ...
      createSession: mock(
        (options: { agentId: string; projectPath: string; title?: string }) =>
          okAsync(
            buildSession(
              "session-1",
              options.agentId,
  ...
  describe("initialize", () => {
    ...
    it("continues startup when preconnection skills warming fails", async () => {
      mockPreconnectionAgentSkillsStore.initialize = mock(() =>
        errAsync(
          new AgentError("skills_list_agent_skills", new Error("Failed")),
        ),
    ...
    it("hydrates restored panels before running the background sidebar scan", async () => {
      ...
      expect(mockSessionStore.loadStartupSessions).toHaveBeenCalledWith([
        "session-1",
      ]);
    ...
    it("clears orphaned restored session ids before attempting startup reconnect", async () => {
      ...
      mockPanelStore.updatePanelSession = mock(
      ...
      expect(mockSessionStore.loadSessions).not.toHaveBeenCalled();
      expect(mockPanelStore.updatePanelSession).toHaveBeenCalledWith(
        "panel-1",
        null,
      );
    ...
    it("preserves recoverable created-session restored ids", async () => {
      ...
      expect(mockPanelStore.updatePanelSession).not.toHaveBeenCalledWith(
        "panel-1",
        null,
      );
    });
