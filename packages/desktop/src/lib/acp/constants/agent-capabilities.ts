...
const AGENT_CAPABILITY_OVERRIDES: Readonly<Record<string, Readonly<Partial<AgentCapabilities>>>> = {
	[AGENT_IDS.CLAUDE_CODE]: {
		openInFinderTarget: "claude-session",
	},
};

export function getAgentCapabilities(agentId: string | null | undefined): AgentCapabilities {
	const overrides =
