...
use super::types::{
    ...
    PlanStep, QuestionData, SessionUpdate, TurnErrorData,
};
use super::usage::parse_usage_telemetry_data;
use crate::acp::agent_context::current_agent;
use crate::acp::parsers::{get_parser, ParseError, UpdateType};
...
impl<'de> serde::Deserialize<'de> for SessionUpdate {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        // Extract session_id from the data if present
        let session_id = raw
            .data
            .get("sessionId")
        ...
        // Determine update type using same logic as TypeScript type-normalizer
        let update_type =
            extract_update_type::<D::Error>(&raw.data, &raw.type_field, &raw.session_update_field)?;

        // Deserialize the specific variant
        match update_type.as_str() {
            "userMessageChunk" => {
                let chunk = deserialize_content_chunk::<D::Error>(&raw.data)?;
            ...
            "plan" => {
                // Dual-format support: agent sends flat format with entries/content,
                // canonical format has nested plan.steps. Try canonical first, then entries fallback.
                // When both keys present, plan wins to avoid ambiguity.
            ...
            "availableCommandsUpdate" => {
                let update_value = require_first_present::<D::Error>(
                    &raw.data,
                    &[
                        "availableCommandsUpdate",
                        "availableCommands",
            ...
            "currentModeUpdate" => {
                ...
                let update: CurrentModeData =
                    serde_json::from_value(update_value).map_err(|e| {
                        serde::de::Error::custom(format!("Invalid current mode: {}", e))
                    })?;
            ...
            "usageTelemetryUpdate" => {
                let data =
                    parse_usage_telemetry_data::<D::Error>(&raw.data, session_id.as_deref())?;
...
/// appropriate parser (ClaudeCodeParser, OpenCodeParser, etc.) based on `current_agent()`.
