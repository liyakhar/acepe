...
/// Apply a provider-owned execution profile for a visible UI mode.
#[tauri::command]
#[specta::specta]
pub async fn acp_set_execution_profile(
    app: AppHandle,
    session_id: String,
    mode_id: String,
    ...
    tracing::debug!(
        ...
        mode_id = %mode_id,
        autonomous_enabled,
        "acp_set_execution_profile called"
    );
    let session_registry = app.state::<SessionRegistry>();
    ...
    let agent_id = session_registry.get_agent_id(&session_id).ok_or_else(|| {
        tracing::error!(
            session_id = %session_id,
            "Session not found for set_execution_profile"
        );
    ...
    let provider = registry.get(&agent_id).ok_or_else(|| {
        tracing::error!(
            ...
            "Provider not found for set_execution_profile"
        );
        SerializableAcpError::AgentNotFound {
            agent_id: agent_id.as_str().to_string(),
        }
    })?;

    let native_mode_id = provider
        .map_execution_profile_mode_id(&mode_id, autonomous_enabled)
    ...
    let mut client_guard =
        lock_session_client(&client_mutex, "acp_set_execution_profile: lock").await?;
    let result = timeout(
        SESSION_CLIENT_OPERATION_TIMEOUT,
        client_guard.set_session_mode(session_id, native_mode_id),
    )
    .await
    .map_err(|_| SerializableAcpError::Timeout {
        operation: "acp_set_execution_profile: operation".to_string(),
    })?
    .map_err(SerializableAcpError::from);
