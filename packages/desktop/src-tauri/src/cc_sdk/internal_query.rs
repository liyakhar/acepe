...
impl Query {
    ...
    pub async fn set_model(&mut self, model: Option<String>) -> Result<()> {
        let req = SDKControlRequest::SetModel(super::types::SDKControlSetModelRequest {
    ...
    pub async fn rewind_files(&mut self, user_message_id: &str) -> Result<()> {
        let req = SDKControlRequest::RewindFiles(super::types::SDKControlRewindFilesRequest::new(
            user_message_id,
        ));
        let _ = self.send_control_request(req).await?;
        Ok(())
