...
impl InteractiveClient {
    ...
    pub async fn get_mcp_status(&mut self) -> Result<Vec<super::types::McpServerStatus>> {
        if !self.connected {
            return Err(SdkError::InvalidState {
                message: "Not connected".into(),
            });
    ...
    pub async fn add_mcp_server(
        ...
        let mcp_msg = super::types::SDKControlMcpMessageRequest {
        ...
        let mut transport = self.transport.lock().await;
        let request = super::types::SDKControlRequest::McpMessage(mcp_msg);
        let json = serde_json::to_value(&request)
            .map_err(|e| SdkError::TransportError(format!("Failed to serialize: {e}")))?;
        let input = super::transport::InputMessage {
    ...
    ) -> Result<Vec<super::sessions::SessionInfo>> {
        super::sessions::list_sessions(directory, limit, include_worktrees).await
    }
    ...
    pub async fn get_session_messages(
        ...
        limit: Option<usize>,
        offset: usize,
    ) -> Result<Vec<super::sessions::SessionMessage>> {
        super::sessions::get_session_messages(session_id, directory, limit, offset).await
    }

    /// Rename a session
    pub async fn rename_session(&self, session_id: &str, title: &str) -> Result<()> {
        super::sessions::rename_session(session_id, title).await
    }

    /// Tag a session
    pub async fn tag_session(&self, session_id: &str, tag: Option<&str>) -> Result<()> {
        super::sessions::tag_session(session_id, tag).await
    }
