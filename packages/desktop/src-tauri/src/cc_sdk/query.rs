...
/// stateful conversations, use [`ClaudeSDKClient`](super::ClaudeSDKClient) instead.
///
...
pub async fn query(
    ...
    match prompt {
        ...
        QueryInput::Stream(_stream) => {
            ...
            Err(super::SdkError::NotSupported {
...
async fn query_print_mode(
    ...
    let cli_path = super::transport::subprocess::find_claude_cli()?;
    ...
    if let Some(ref prompt_v2) = options.system_prompt_v2 {
        match prompt_v2 {
            super::types::SystemPrompt::String(s) => {
                cmd.arg("--system-prompt").arg(s);
            }
            super::types::SystemPrompt::Preset { append, .. } => {
    ...
    if let Some(ref thinking) = options.thinking {
        match thinking {
            super::types::ThinkingConfig::Enabled { budget_tokens } => {
                cmd.arg("--max-thinking-tokens")
                    .arg(budget_tokens.to_string());
            }
            super::types::ThinkingConfig::Disabled => {
                // Don't pass thinking tokens flag
            }
            super::types::ThinkingConfig::Adaptive => {
    ...
    if let Some(user) = options.user.as_deref() {
        super::transport::subprocess::apply_process_user(&mut cmd, user)?;
    }

    let mut child = cmd.spawn().map_err(super::SdkError::ProcessError)?;

    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| super::SdkError::ConnectionError("Failed to get stdout".into()))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| super::SdkError::ConnectionError("Failed to get stderr".into()))?;
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            match serde_json::from_str::<serde_json::Value>(&line) {
                Ok(json) => {
                    match super::message_parser::parse_message(json) {
                        Ok(Some(message)) => {
        ...
        match child.wait().await {
            Ok(status) => {
                if !status.success() {
                    let _ = tx
                        .send(Err(super::SdkError::ProcessExited {
                            code: status.code(),
                        }))
                        .await;
                }
            ...
            Err(e) => {
                let _ = tx.send(Err(super::SdkError::ProcessError(e))).await;
            }
        }
    });
