...
///
/// ```rust,no_run
/// use acepe_lib::cc_sdk::{ClaudeSDKClient, ClaudeCodeOptions, Message, Result};
...
impl ClaudeSDKClient {
    ...
    ///
    /// ```rust,no_run
    /// # use acepe_lib::cc_sdk::{ClaudeSDKClient, ClaudeCodeOptions, SubprocessTransport};
    /// # fn example() {
            ...
            || options.enable_file_checkpointing
        {
            ...
            let sdk_mcp_servers: HashMap<String, Arc<dyn std::any::Any + Send + Sync>> = options
                ...
                .filter_map(|(k, v)| {
                    ...
                    if let super::types::McpServerConfig::Sdk { name: _, instance } = v {
    ...
    pub async fn send_user_message(&mut self, prompt: String) -> Result<()> {
        ...
        self.send_user_message_with_session(prompt, session_id)
            .await
    }
    ...
    pub async fn send_request(&mut self, prompt: String, session_id: Option<String>) -> Result<()> {
        let effective_session_id = session_id
            .or_else(|| self.options.session_id.clone())
            .unwrap_or_else(|| "default".to_string());
        self.send_user_message_with_session(prompt, effective_session_id)
            .await
    }
    ...
    pub async fn get_usage_stats(&self) -> super::token_tracker::TokenUsageTracker {
    ...
    pub async fn set_budget_limit(
        &self,
        limit: super::token_tracker::BudgetLimit,
        on_warning: Option<super::token_tracker::BudgetWarningCallback>,
    ) {
